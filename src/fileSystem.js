...
function readWatchingListJSON(isolated, safe, type) {
    ...
    if (type === MOVIE) { } 
        ...
        const rawData = fs.readFileSync(`./lib/server/profiles/${userName}Data/watchingListTV.json`);
        let watchingList = {};
...
function writeWatchingListJSON(data, safe, type) {
    ...
    if (type === MOVIE) { } 
        ...
        fs.writeFileSync(`./lib/server/profiles/${userName}Data/watchingListTV.json`, JSON.stringify(watchingList));
    }
}
...
function readListJSON(safe, type) {
    ...
    if (type === MOVIE) { 
        const rawData = fs.readFileSync(`./lib/server/profiles/${userName}Data/listMovie.json`);
        let list = [];
        ...
        list.splice(index, 1);
        fs.writeFileSync(`./lib/server/profiles/${userName}Data/listMovie.json`, JSON.stringify(list));
        
        return { content: content, type: 'movie' }
    } 
    else if (type === TV) {
        const rawData = fs.readFileSync(`./lib/server/profiles/${userName}Data/listTV.json`);
        let list = [];
        try { list = JSON.parse(rawData); } catch (e) { }
        const index = Math.floor(Math.random() * list.length);
        const content = list[index].replace(/\(\d\d\d\d\)/g, '');
        ...
        list.splice(index, 1);
        fs.writeFileSync(`./lib/server/profiles/${userName}Data/listTV.json`, JSON.stringify(list));
        ...
        const rawData = fs.readFileSync(`./lib/server/profiles/${userName}Data/list${file}.json`);
